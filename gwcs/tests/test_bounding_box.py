...
class TestCompoundBoundingBox:
    ...
    def test_validate(self):
        # Main validate
        bbox = {1: (-1, 0), 2: (0, 1)}
        bounding_box = CompoundBoundingBox.validate(Gaussian2D(), bbox, slice_args=['x', ('y', True)])
        assert bounding_box == bbox
        ...
        # Re validate
        new_bounding_box = CompoundBoundingBox.validate(Gaussian2D(), bounding_box)
        assert new_bounding_box == bounding_box
        assert new_bounding_box._slice_args == bounding_box._slice_args

        # Simple validate
        bbox = {1: (-1, 0), 2: (0, 1)}
        bounding_box = CompoundBoundingBox.validate(Gaussian2D(), bbox, slice_args=[('y', True)])
        assert bounding_box == bbox
        for bbox_slice in bounding_box.values():
            assert isinstance(bbox_slice, BoundingBox)
        assert (bounding_box._model.parameters == Gaussian2D().parameters).all()
