...
def test_angles_grating_equation():
    """Test against the Nirspec implementation."""
    lam = np.array([2e-6] * 4)
    ...
    x_out = -alpha_in - (model.groove_density * model.spectral_order * lam)

    alpha_out, beta_out, gamma_out = model(lam, -alpha_in, alpha_in)
    assert_allclose(alpha_out, x_out)
    ...
    # Eq. from Nirspec model.
    x_out = -alpha_in - (model.groove_density * model.spectral_order * lam * u.m)
...
def test_snell_sellmeier_combined(sellmeier_glass):
    ...
    model = sellmeier_glass & to_dir_cos | sp.Snell3D() & Identity(1) | from_dir_cos
