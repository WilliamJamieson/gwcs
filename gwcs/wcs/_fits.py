...
from gwcs._typing import Bbox, Real
...
class FitsMixin(BaseGwcs):
    """
    This mixin pulls all the FITS-specific functionality into a separate class.
    """
    ...
    def to_fits_sip(
        ...
        """
        _, _, celestial_group = self._separable_groups(detect_celestial=True)  # type: ignore[no-untyped-call]
    ...
    def _to_fits_sip(
        ...
        new_cards = [
            Card(keyword=axis_rename[c.keyword], value=c.value, comment=c.comment)
            if c[0] in axis_rename
            else c
            for c in hdr.cards
        ]

        hdr = Header(new_cards)
        hdr["WCSAXES"] = 2
    ...
    def to_fits_tab(
        ...
        if bounding_box is None:
            ...
            ModelBoundingBox.validate(transform_0, bounding_box)
    ...
    def _to_fits_tab(
        ...
        bin_table_hdu = BinTableHDU(arr, name=bin_ext[0], ver=bin_ext[1])
